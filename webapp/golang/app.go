...
// digest は文字列の SHA-512 ハッシュを計算します。
...
// calculateSalt はアカウント名からソルト（ハッシュ）を計算します。
func calculateSalt(accountName string) string {
	return digest(accountName)
}

// calculatePasshash はパスワードとソルトを組み合わせてパスワードハッシュを生成します。
func calculatePasshash(accountName, password string) string {
	salt := calculateSalt(accountName)
...
func getIndex(w http.ResponseWriter, r *http.Request) {
	...
	err := db.Select(&results, "SELECT p.id,p.user_id,p.body,p.created_at,p.mime,u.account_name FROM `posts` AS p "+
		"JOIN `users` AS u ON p.user_id=u.id"+
		"WHERE u.del_flg = 0 ORDER BY p.created_at DESC LIMIT 21")
	if err != nil {
		log.Print(err)
		return
	}
